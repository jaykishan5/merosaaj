...
export default function ProfileSettingsModal({ isOpen, onClose, onUpdate }: ProfileSettingsModalProps) {
    ...
    return (
        <Dialog open={isOpen} onOpenChange={(open) => !open && onClose()}>
            <DialogContent className="max-w-[480px] w-[95vw] px-0 py-0 overflow-hidden bg-background">
                <DialogHeader className="bg-background border-b border-border/50 px-8 py-6">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            <DialogTitle className="text-xl font-bold tracking-tight font-serif text-primary">Personal Profile</DialogTitle>
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider">Update your digital identity</p>
                        </div>
                    </div>
                    ...
                    {fetching ? (
                        ...
                        <form onSubmit={handleSubmit} className="space-y-10">
                            ...
                            <div className="flex flex-col items-center">
                                ...
                                >
                                    <div className="w-32 h-32 rounded-[2.5rem] border-4 border-white overflow-hidden relative transition-all duration-500 group-hover:scale-105 group-hover:rounded-[2rem] shadow-2xl bg-muted ring-1 ring-border/50">
                                        {isUploading ? (
                                        ...
                                        {formData.image ? (
                                            ...
                                            <div className="w-full h-full flex items-center justify-center bg-accent/5">
                                                <User className="w-14 h-14 text-accent/20" />
                                            </div>
                                        )}
                                        ...
                                        <div className="absolute inset-0 bg-accent/10 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center backdrop-blur-[2px]">
                                            <div className="bg-white/90 p-3 rounded-2xl shadow-xl flex items-center gap-2">
                                                ...
                                                <span className="text-[10px] font-bold uppercase tracking-widest text-accent">Update</span>
                                            </div>
                                        </div>
                                    </div>
                                    <div className="absolute -bottom-2 -right-2 bg-accent text-white p-3 rounded-2xl shadow-xl border-4 border-white z-10 transition-transform group-hover:scale-110 group-hover:rotate-6">
                                        <Camera className="w-4 h-4" />
                                    </div>
                                </div>
                                <p className="mt-5 text-[9px] font-bold text-muted-foreground uppercase tracking-[0.2em] font-outfit">Click photo to update</p>
                            </div>

                            <div className="grid grid-cols-1 gap-6">
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground pl-1">Display Name</label>
                                ...
                                <div className="space-y-3">
                                    <label className="text-[10px] font-bold uppercase tracking-[0.2em] text-muted-foreground pl-1">Phone Number</label>
                                    <div className="relative group">
                                        <div className="absolute left-5 top-1/2 -translate-y-1/2 transition-colors duration-300 group-focus-within:text-accent">
                            ...
                            <button
                                type="submit"
                                disabled={loading}
                                className="w-full py-5 bg-primary text-white font-bold rounded-2xl uppercase tracking-[0.2em] hover:bg-accent hover:shadow-2xl hover:shadow-accent/20 transition-all duration-500 disabled:opacity-50 active:scale-[0.98] flex items-center justify-center gap-3 relative overflow-hidden group font-outfit mt-8 shadow-xl shadow-primary/10"
                            >
