...
import { Heart, Share2, Star, MapPin, Truck, RotateCcw, ShieldCheck, Smartphone, MessageSquare, Package as PackageIcon, Zap } from "lucide-react";
...
export default function ProductDetailsClient({ product: initialProduct }: { product: any }) {
    ...
    const allColors = product ? Array.from(new Set(product.variants.flatMap((v: any) =>
        v.color.split(',').map((c: string) => c.trim())
    ))) : [];

    const allSizes = product ? Array.from(new Set(product.variants.flatMap((v: any) =>
        v.size.split(',').map((s: string) => s.trim())
    ))) : [];

    const currentVariant = product ? product.variants.find((v: any) =>
