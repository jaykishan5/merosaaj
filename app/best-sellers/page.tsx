...
async function getBestSellers() {
    await dbConnect();
    // For now, let's just return featured products as best sellers
    return await Product.find({ isFeatured: true }).limit(8).lean();
}

export default async function BestSellersPage() {
    const products: any = await getBestSellers();
    ...
    return (
        <main className="min-h-screen">
            ...
            <section className="py-24 px-4 max-w-7xl mx-auto">
                <div className="mb-12">
                    <h1 className="text-5xl font-bold tracking-tighter italic">BEST SELLERS</h1>
                    <p className="text-muted-foreground mt-2">The most wanted pieces from our collection.</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                    {products.length > 0 ? (
                        ...
                        <div className="col-span-full py-20 text-center border border-dashed border-border rounded-xl">
                            <p className="text-muted-foreground">Stay tuned for our upcoming best sellers.</p>
                        </div>
                    )}
                </div>
            </section>
