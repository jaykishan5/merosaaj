...
export default async function Home() {
    ...
    return (
        <main className="min-h-screen bg-[#FDFDFD]">
            ...
            {/* Category Strip (Bewakoof Circle Style) */}
            <section className="py-12 bg-white border-b border-gray-50 relative z-30">
                <div className="max-w-7xl mx-auto px-4 overflow-visible">
                    <div className="flex gap-8 md:justify-center overflow-x-auto pt-10 pb-10 scrollbar-none px-4">
                        {categories.map((cat) => (
                            <Link
                                href={cat.href}
                                key={cat.name}
                                className="flex flex-col items-center gap-4 shrink-0 group transition-all duration-500"
                            >
                                <div className="relative">
                                    ...
                                    {/* Main Icon Circle */}
                                    <div className="w-16 h-16 md:w-24 md:h-24 rounded-full bg-[#f8f8f8] flex items-center justify-center text-3xl md:text-5xl border-2 border-transparent group-hover:border-accent group-hover:scale-105 group-hover:shadow-[0_20px_40px_rgba(0,0,0,0.1)] transition-all duration-500 relative z-10 overflow-hidden">
                                ...
                                <span className="text-[9px] md:text-[10px] font-black uppercase tracking-[0.2em] text-muted-foreground group-hover:text-primary group-hover:translate-y-1 transition-all text-center">
                                    {cat.name}
                                </span>
                            </Link>
                        ))}
            ...
            <section className="py-24 overflow-hidden">
                <div className="max-w-7xl mx-auto px-4 mb-12 flex justify-between items-end">
                    <div className="space-y-2">
                        ...
                        <h2 className="text-4xl md:text-6xl font-black tracking-tighter italic">HOTTEST DROPS</h2>
                    </div>
                    <Link href="/shop" className="group flex items-center gap-4 text-xs font-black uppercase tracking-[0.2em] border-b-2 border-primary pb-2">
                        View More
                        <ArrowRight className="w-4 h-4 group-hover:translate-x-2 transition-transform" />
                    </Link>
                </div>

                <div className="flex gap-6 overflow-x-auto pb-12 px-4 md:px-[calc((100vw-1280px)/2+1rem)] scrollbar-none snap-x snap-mandatory overflow-visible">
                    {products.length > 0 ? (
                        products.map((product: any) => (
                            <ProductCard key={product._id} product={product} />
                        ))
                    ) : (
                        <div className="w-full py-20 text-center border-4 border-dashed border-muted rounded-[3rem]">
            ...
            <section className="py-24 bg-primary text-white">
                <div className="max-w-7xl mx-auto px-4">
                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
                        <div className="lg:col-span-2 relative h-[500px] rounded-[3rem] overflow-hidden group cursor-pointer shadow-2xl">
                            <Image
                            ...
                            <div className="absolute inset-0 flex flex-col justify-end p-12">
                                <span className="text-accent font-black uppercase tracking-[0.4em] mb-4">Identity Collection</span>
                                <h3 className="text-6xl font-black tracking-tighter mb-8 italic">MODERN <br /> STREETWEAR</h3>
                                <Link href="/shop" className="bg-white text-primary w-fit px-12 py-5 rounded-2xl font-black text-xs uppercase tracking-[0.2em] shadow-xl">Shop Now</Link>
                            </div>
                        </div>
